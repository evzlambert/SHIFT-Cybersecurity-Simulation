"""
SHIFT Cybersecurity Breach Simulation — Student Response App v2
...
v2 change: auto-saves team progress to SHARED_DIR on every decision and
station completion so the ICC dashboard updates without manual file transfer.

Local simulation (Option A):
    Run on instructor laptop — all teams connect via browser at
    http://[laptop-ip]:8502. Saves land in SHARED_DIR on the same machine
    that icc_dashboard.py reads from.

Launch:  python3 -m streamlit run student_app_v2.py --server.port 8502
Config:  SHIFT_SHARED_DIR env var overrides the default submissions/ folder
"""
...
# ---------------------------------------------------------------------------
# Shared directory — auto-save target for ICC dashboard (Option A setup)
# Override with SHIFT_SHARED_DIR env var to point at a network-accessible path.
# Defaults to a submissions/ folder next to this script.
# ---------------------------------------------------------------------------
...
# Keep legacy name pointing at the same location so PDF export still works
SUBMISSIONS_DIR = SHARED_DIR
...
def _auto_save():
    """Write team progress to SHARED_DIR so the ICC dashboard updates live."""
    team = st.session_state.get("team_name", "").strip()
    if not team:
        return
    try:
        payload = {k: st.session_state.get(k) for k in _AUTO_SAVE_KEYS
                   if k in st.session_state}
        path = os.path.join(SHARED_DIR, f"SHIFT_{team}.json")
        with open(path, "w") as f:
            json.dump(payload, f, default=str)
    except Exception:
        pass  # never break the simulation if auto-save fails
